...
def pick(paragraphs, select, k):    # 将根据要求筛选出符合要求的单词
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ans = [x for x in paragraphs if select(x)]  # 首先将符合select要求的词筛选出来 （这种形式叫做列表推导式）
    if (k + 1 > len(ans)):  # 进行判断，如果需要选择的个数大于答案的长度，直接返回空字符
        return ''
    else :
        return ans[k]
    # END PROBLEM 1
...
def about(subject): # 包含一个单词列表一个主题，含有哪个主题进行输出
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def about2(text):   # 定义内置函数
        text1 = remove_punctuation(text)    # 调用移除标点符号函数
        text2 = split(text1)    # 调用变成单词列表函数
        ans1 = [lower(x) for x in text2]    
        ans2 = [lower(x) for x in subject]
