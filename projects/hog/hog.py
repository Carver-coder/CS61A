...
def roll_dice(num_rolls, dice=six_sided):  # 投掷骰子的次数and一个回合得到的点数
...
def boar_brawl(player_score, opponent_score): #根据规则如果投掷0个骰子所返回的结果
    ...
    if opponent_score >= 0 and opponent_score <= 9:
        tens = 0 
    else :
        tens = opponent_score // 10 % 10
    answer = 3 * (abs(ones - tens))
    if answer == 0:
        return 1
    else :
        return answer
...
def make_averaged(original_function, samples_count=1000):
    ...
    def av(*args):
        ...
        for i in range(samples_count):
            counts += original_function(*args)
        return counts / samples_count
    return av
    # END PROBLEM 8
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    sum = boar_brawl(score, opponent_score)
    if sum >= threshold:
        return 0
    else :
        return num_rolls
